<?php
	$member_password = filter_input(INPUT_POST, 'first');
	$member_id = $member -> getMember_Id();
	
	$stmt_edit_password = $mysqli -> prepare("UPDATE member SET member_password=? WHERE member_id=?");
	$stmt_edit_password -> bind_param('si', $member_password, $member_id);
	...
	$stmt_select_member -> bind_result($old_member_name, $old_member_email, $old_member_ic, $old_member_bank_id, $old_member_bank_acc_number, $old_member_acc_name, $old_member_password);
	while($stmt_select_member -> fetch()){
		if($stmt_edit_password -> execute()){
			...
			$stmt_update_member_ae -> bind_param('isssisss', $member_id, $old_member_name, $old_member_email, $old_member_ic, $old_member_bank_id, $old_member_bank_acc_number, $old_member_acc_name, $old_member_password);
